...
function BarRow({ label, you, market, sev, unit = '%' }: {
  label: string; you: number; market: number; sev: string; unit?: string;
}) {
...
function ActionItem({ n, sev, title, body, savings, clause, lenses = [] }: {
  n: string; sev: string; title: string; body: string; savings?: string; clause: string; lenses?: string[];
}) {
  ...
  return (
    <div className="card" style={{ padding: '14px 16px', marginTop: 10 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
        ...
        {savings && <span className="num" style={{ fontSize: 13.5, color: 'var(--green)', fontWeight: 600 }}>{savings}</span>}
      </div>
      {lenses.length > 0 && (
...
function IntlRefCard({ tag, body }: { tag: string; body: string }) {
  return (
    ...
    <AppShell activeNav="Análisis" rightExtra={
      ...
      <div className="coach-page" style={{ padding: '22px 32px 0' }}>
        ...
        <LensScorecard cards={[
          ...
          { lens: 'intl', status: 'ref', headline: '3 observaciones', summary: 'Bajo CFPB el cambio de plazo podría ser práctica engañosa. No genera alertas.', count: 'solo referencia' },
        ]} />
      </div>
      ...
      <div className="coach-page coach-main-grid" style={{ padding: '20px 32px 0', display: 'grid', gridTemplateColumns: '1.35fr 1fr', gap: 18 }}>
        ...
        >
          ...
          <BarRow label="Tasa de interés" you={18.4} market={15.9} sev="hi" />
          <BarRow label="Comisión inicial" you={2.1} market={1.4} sev="mid" />
        </SectionCard>
      ...
      <div style={{
        ...
        <div style={{ flex: 1 }} />
        <button className="btn"><Icon name="file" size={14} /> Exportar PDF</button>
        <button className="btn"><Icon name="compare" size={14} /> Comparar</button>
